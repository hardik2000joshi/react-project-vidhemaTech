import { useState } from 'react'
import reactLogo from './assets/react.svg'
import viteLogo from './assets/vite.svg'
import heroImg from './assets/hero.png'
import "./App.css";
import { UICard } from './components/Card';
import { CreateArray } from './components/array';


export function Abc(){
  console.log("React is library of javascript- used to build complex UI making dynamic interactive site");
  return <h1>
    React is library of javascript- used to build complex UI making dynamic interactive site
  </h1>
}

export let Message = () => {
  return "Hello World";
}

// export let Design = () => {
//   return (
//     <div>
//     <Card admin="Rahul" age={22} />
//     <Card admin="Yash" age={24}/>
//     <Card admin="Mohit" age={19}/>
//         </div>
//   )
// }

export let Design = () => {

  const arrayDesign = [{
    name: "Amazon",
    image: "https://1000logos.net/wp-content/uploads/2016/10/Amazon-logo-meaning.jpg",
    role:"Senior UI/UX Designer", 
    duration:"Part Time", 
    level:"Junior Level"
  },
  {
    name:"Swiggy", 
      image: "https://upload.wikimedia.org/wikipedia/commons/1/13/Swiggy_logo.png" ,
      role: "Full Stack Web Developer", 
      duration: "Full Time",
      level: "Senior Level"
  },
  {
    name:"Flipkart",
    image:"https://media.licdn.com/dms/image/v2/D5622AQHndUsRI-xNTw/feedshare-shrink_800/feedshare-shrink_800/0/1733728700313?e=2147483647&v=beta&t=Mdzx1f5esSaJS4pqKrI2P51B1921hgdVDItVdMVqr_U",
    role:"Business Analyst", 
      duration:"Full Time",
      level:"Mid Level"
  },
  {
    name:"Zomato", 
      image:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT8nRNfM_vH-KXfVsrYTz7rctEhrRscF1eobm5gzo4zYX8FbFJIJ63JMU4X&s=10", 
      role:"Frontend Web Developer",
      duration:"Part Time",
      level:"Junior Level"
  },
  {
     name:"Uber",
      image:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRhdxGY8Rd6O9bVufCIB_8bmV-KbaSxxCoVZzfoXEmQ8A&s=10",
      role:"SEO Developer",
      duration:"Full Time",
      level:"Senior Level"
  },
  {
    name:"Urban Clap",
      image:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTecm1wcvmWmbqEdOmKYokUQFoikhL6mvPOOU5xWIQqrBNeNRveUND9iaPD&s=10",
      role:"Backend Developer",
      duration:"Part Time",
      level:"Mid Level"
  },
]
console.log(arrayDesign);
  return(
    <div>
    <div className="row">
      
      {arrayDesign.map((arr, index)=> {
        return(
          <div key={index}>
            <UICard name={arr.name} image = {arr.image} role={arr.role} duration={arr.duration} level={arr.level}/>
          </div>
        )
      })}

      {/* <UICard 
      name="Amazon" 
      image="data:image/png;base64," 
      role="Senior UI/UX Designer" 
      duration="Part Time" 
      level="Junior Level"
      />

      <UICard 
      name="Swiggy" 
      image="https://upload.wikimedia.org/wikipedia/commons/1/13/Swiggy_logo.png" 
      role="Full Stack Web Developer" 
      duration = "Full Time"
      level = "Senior Level"
      />

      <UICard 
      name="Flipkart" 
      image="https://media.licdn.com/dms/image/v2/D5622AQHndUsRI-xNTw/feedshare-shrink_800/feedshare-shrink_800/0/1733728700313?e=2147483647&v=beta&t=Mdzx1f5esSaJS4pqKrI2P51B1921hgdVDItVdMVqr_U" 
      role="Business Analyst" 
      duration="Full Time"
      level = "Mid Level"
      />

      </div>

      <div className='row'>
      <UICard 
      name="Zomato" 
      image="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT8nRNfM_vH-KXfVsrYTz7rctEhrRscF1eobm5gzo4zYX8FbFJIJ63JMU4X&s=10" 
      role="Frontend Web Developer" 
      duration = "Part Time"
      level = "Junior Level"
       />
      <UICard 
      name="Uber" 
      image="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRhdxGY8Rd6O9bVufCIB_8bmV-KbaSxxCoVZzfoXEmQ8A&s=10" 
      role="SEO Developer" 
      duration = "Full Time"
      level = "Senior Level"
      />
      <UICard 
      name="Urban Clap" 
      image="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTecm1wcvmWmbqEdOmKYokUQFoikhL6mvPOOU5xWIQqrBNeNRveUND9iaPD&s=10" 
      role="Backend Developer" 
      duration = "Part Time"
      level = "Mid Level"
      /> */}

      </div>
      <CreateArray />
    </div>
  )
}

function App() {
  // const [count, setCount] = useState(1);


  return (
    <div>

        <div>
          <h1>NPM - Node Package Manager</h1>
          <p>
            NPM is a package manager which is required to be included in project to run the project
          </p>
        </div>
        <button
          type="button"
          className="counter"
          // onClick={() => setCount((count) => count + 2)}
        >
          Count is 1
        </button>

      <div>
        <h1>
          React is library of javascript
        </h1>

        <form action="">
          Name: <input type="text" /> <br /> <br />
          email: <input type="email" /> <br /> <br />
          Address: <input type="text" /> <br /> <br />
          Password: <input type="Password" /> <br /> <br />
          Gender: <br /> <br />
          <input type="checkbox" /> Male <input type="checkbox" /> female <input type="checkbox" /> others <br /> <br />
          <input type="submit" /> <br /> <br />
        </form>
      </div>

      
    </div>
  )
}

export default App;
